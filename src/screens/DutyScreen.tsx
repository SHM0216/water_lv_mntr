...
export function DutyScreen({ navigation }: Props) {
  ...
  const sorted = [...duty].sort((a, b) => (a.date === b.date ? a.shift.localeCompare(b.shift) : b.date.localeCompare(a.date)));
  ...
  return (
    <View style={styles.root}>
      ...
      <View style={styles.formCard}>
        ...
        <Row>
          <Field label="날짜" value={form.date} onChange={(v) => setForm({ ...form, date: v })} placeholder="YYYY-MM-DD" />
          <ShiftToggle value={form.shift} onChange={(v) => setForm({ ...form, shift: v })} />
        </Row>
        <Row>
          <Field label="주 책임자" value={form.primary} onChange={(v) => setForm({ ...form, primary: v })} placeholder="예) 김주임" />
          <Field label="백업" value={form.backup} onChange={(v) => setForm({ ...form, backup: v })} placeholder="예) 박대리" />
        </Row>
        <Row>
        ...
        <Field
          ...
          placeholder="야간 집중 호우 예보 · 강남·서초 중점 모니터링 등"
          multiline
        />
...
function ShiftToggle({ value, onChange }: { value: 'day' | 'night'; onChange: (v: 'day' | 'night') => void }) {
  return (
...
const styles = StyleSheet.create({
  ...
  formRow: { flexDirection: 'row', marginBottom: spacing.sm },
